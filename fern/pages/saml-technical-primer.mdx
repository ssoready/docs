...
Most likely, you'll first hear about SAML because your customer wants your
product to support it. Customer requests are the most common initial reason
companies add support for SAML. But why does your customer want SAML support?
...
secure. Concretely, the biggest things they worry about include:
...
the identity provider will stop letting that employee do SAML-based logins into *anything*. The fired employee is locked out of every
work application.
...
<Info>
    The "off-board employees after being fired" objective also pushes your
    customers toward wanting support for
    [SCIM](https://en.wikipedia.org/wiki/System_for_Cross-domain_Identity_Management).
    SAML lets your customers enforce "fired employees can't log in again", and
    SCIM additionally lets your customers enforce "fired employees' accounts are
    automatically deactivated".

    The rest of this article is all about SAML, but [SSOReady can help you
    implement SCIM, too.](/docs/scim/scim-quickstart).
</Info>
...
* You haven't previously processed a SAML assertion with the same `ID`:

  ```xml {1}
  <saml2:Assertion ID="id23923151778251751045676989" ...>
    ...
  </saml2:Assertion>
  ```

  <Warning>
      Checking that every SAML assertion's `ID` is distinct guards against
      replay attacks.

      A SAML assertion is, in essence, a bearer token. That bearer token has a
      limited validity window (which you verified previously in this section),
      but it also gets handled by many parties. There's a risk that one of the
      parties accidentally leaks those assertions, and an attacker could replay
      a leaked assertion before the validity window expires.

      The SAML specification requires that every `Assertion` have an `ID`
      attribute, and that this ID have a "negligible probability" of collision.
      By ensuring that you never process two assertions with the same `ID`, you
      make replay attacks much more difficult for attackers -- they must not
      only intercept the assertion, but send it to you *faster* than the victim.
      This is a very difficult attack to carry out in practice, because usually
      only milliseconds pass between an IDP's issuance of an assertion and your
      customer presenting it to you.

      SSOReady always verifies assertion ID uniqueness. This functionality
      cannot be disabled.
  </Warning>
...
* The SAML assertion isn't being repeated by an imposter
