...
<Info>
    ...
    (i.e. the `foo` in `foo:bar`, not just what `foo` resolves to) elements and
    attributes use. These details are often abstracted away, because they don't
...
XML Canonicalization rather involved, but the basic idea is to make details that
don't affect message semantics always resolve to the same thing:
...
domains in your system. Unless they own a domain -- something *you* control, not
your customer -- then they can't do SAML logins into that domain.
