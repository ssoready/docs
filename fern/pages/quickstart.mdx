...
API keys are scoped to an environment. When viewing an environment in the app, the "Create API Key" button creates a new
API key. A popup will show you your new API key's secret (it starts with `ssoready_sk_...`). That's the API key you'll
