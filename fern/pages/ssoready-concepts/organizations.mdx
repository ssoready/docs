...
<Note> We recognize that some companies will find it messy to assign unique identifiers to customers. For example, you might need to decide whether Salesforce and Slack represent distinct entities. We **strongly** recommend that you maintain whatever convention your production application uses. </Note>
...
As a security precaution, SSOReady allows logins for the Organization if and **only if** the domain it pulls from a users' email address exists among the domains you listed for that Organization. 
