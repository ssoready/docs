...
services:
  ...
  ssoready-auth:
    image: ssoready/ssoready-auth:sha-2f1ce3d
    environment:
      AUTH_SERVE_ADDR: ":80"
  ...
  ssoready-api:
    image: ssoready/ssoready-api:sha-2f1ce3d
    environment:
      API_SERVE_ADDR: ":80"
  ...
  ssoready-app:
    image: ssoready/ssoready-app:sha-2f1ce3d
    environment:
      APP_SERVE_PORT: "80"
...
```bash
docker run --network=host ssoready/ssoready-migrate:sha-2f1ce3d -d 'postgres://postgres:password@localhost/postgres?sslmode=disable' up
```
